    ...
    <header class="editorial-page site-header pb-0" aria-label="Site header">
      <div class="frosted-shell header-shell relative flex min-h-[58px] flex-wrap items-center justify-between gap-x-5 gap-y-2 px-4 sm:px-5">
        ...
        <div class="header-actions">
          <nav class="flex gap-1 overflow-x-auto rounded-full border border-white/70 bg-white/40 p-1 font-mono text-[10px] uppercase tracking-[.1em] text-text-muted shadow-[0_10px_24px_-16px_rgb(6_6_9/.5),inset_0_1px_rgb(255_255_255/.85)] backdrop-blur-[20px] backdrop-saturate-[180%]" aria-label="Primary navigation">
            <Link to="/" preload="intent" activeOptions={{ exact: true }} class={NAV_LINK_CLASS} activeProps={{ class: NAV_LINK_ACTIVE_CLASS }}>Home</Link>
            <Link to="/explore" preload="intent" search={makeBrowseSearch()} class={NAV_LINK_CLASS} activeProps={{ class: NAV_LINK_ACTIVE_CLASS }}>Explore</Link>
          ...
          <span class="hidden items-center gap-[7px] font-mono text-[10px] uppercase tracking-[.13em] text-text-muted md:inline-flex"><i class="inline-block size-[5px] animate-pulse rounded-full bg-emerald shadow-[0_0_8px_rgb(0_200_83/.6)]" />Catalog live</span>
          <GlobalSearch />
        </div>
      </div>
    </header>
