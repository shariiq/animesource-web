...
import { chapterDisplayNumber } from '../manga/reader/createMangaReaderSession'
      ...
      <Show when={props.items.length > 0} fallback={<p class="px-5 py-8 text-sm text-text-secondary sm:px-6">Your reading history will appear here after a manga chapter starts.</p>}>
        <div class="divide-y divide-line">
          <For each={props.items}>{(item) => {
            ...
            const chapterNumber = () => chapterDisplayNumber({ number: item.chapterNumber, title: item.chapterTitle })
            const chapterLabel = () => chapterNumber() === null ? item.chapterTitle || 'Unnumbered chapter' : `Chapter ${chapterNumber()}`
            return (
              <article class="grid grid-cols-[88px_minmax(0,1fr)] gap-4 p-4 sm:grid-cols-[92px_minmax(0,1fr)_auto] sm:items-center sm:p-5">
                <div class="relative aspect-[2/3] w-full self-start overflow-hidden rounded-[8px] bg-violet/12">
                  ...
                  <span class="absolute bottom-1 left-1 rounded-[4px] bg-ink/80 px-1.5 py-1 font-mono text-[8px] uppercase text-white">Ch {chapterNumber() ?? '—'}</span>
                </div>
                <div class="min-w-0">
                  <p class="mono-signal">{item.sourceName || item.sourceId} · {chapterLabel()}</p>
                ...
                <div class="col-span-2 flex flex-wrap gap-2 sm:col-span-1 sm:justify-end">
                  <Link class="ink-control inline-flex items-center px-3 py-2 no-underline" to="/manga/$mangaId/read/$chapterNumber" params={{ mangaId: String(item.anilistId), chapterNumber: `id:${item.chapterId}` }} search={{ source: item.sourceId }}>{item.completed ? '↺ Read again' : '▶ Resume'}</Link>
