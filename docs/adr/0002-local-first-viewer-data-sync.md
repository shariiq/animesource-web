...
AnimeSource already has a validated, browser-only IndexedDB implementation for favorites, Continue Watching, playback progress, source matches, and playback preferences. Phase 3 needs account-backed data without making offline playback depend on a network request or coupling route components to a provider SDK.
