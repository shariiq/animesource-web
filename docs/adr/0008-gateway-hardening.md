...
2. **Unify upstream auth.** Ticket-mode asset fetches carry the service Bearer like catalog, replay the preserved upstream signed query, and best-effort bind a session playback capability when secrets resolve. Direct mode is unchanged (fail-closed capabilities). A joint contract test pins both modes.
