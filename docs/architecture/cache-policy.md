...
The AniList client owns 429 handling: it honors `Retry-After` and `X-RateLimit-Reset`, banks an exhausted `X-RateLimit-Remaining: 0` window into a short shared cooldown, treats GraphQL `Too Many Requests` errors as rate limits, and performs at most one rate-limit retry. Query-level retries do not repeat a final 429 or a cancelled request; other failures receive at most one query retry. Query functions pass TanStack Query's abort signal through to the transport so abandoned route or typeahead work can stop.
