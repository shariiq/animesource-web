...
| `app/data/anisource` | Client-only REST transport, timeout/error mapping, response schemas, subtitle normalization | AniList metadata and route loaders |
...
- AniSource is only reached by the Watch session after the route mounts in a browser. It is never a discovery loader or prefetch.
