...
- **AniSource** — `/health` with bounded cold-start polling (4 attempts × 8s; retries only transient statuses), `/api/v1/sources` (must return ≥1 source), and a real search on the first source.
